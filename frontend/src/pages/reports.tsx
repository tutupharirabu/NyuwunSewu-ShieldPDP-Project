...
export function ReportsPage() {
  const { data, loading, error, refresh } = useApi(async () => {
    const [reports, projects, scans] = await Promise.all([api.reports(), api.projects(), api.scans()]);
  ...
  if (error || !data) return <EmptyState title="Reports unavailable" description={error ?? "Unable to load reports."} />;
  ...
  async function generateReport() {
    ...
    try {
      ...
      setActionError(exc instanceof Error ? exc.message : "Unable to generate report");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function download(reportId: string, extension: string) {
    ...
    try {
      ...
      setActionError(exc instanceof Error ? exc.message : "Unable to download report");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function deleteReport(reportId: string) {
    const confirmed = window.confirm("Delete this generated report? This only removes the report export, not scans or findings.");
    if (!confirmed) return;
    ...
    try {
      ...
      setActionError(exc instanceof Error ? exc.message : "Unable to delete report");
    } finally {
      setBusy(null);
    }
  }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Create audit-ready executive, technical, compliance, or remediation reports.</CardDescription>
        </CardHeader>
        <CardContent>
          {notice && (
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_1fr_220px_160px_auto] lg:items-end">
              ...
              <Select value={scanId} onChange={(event) => setScanId(event.target.value)}>
                ...
                {projectScans.map((scan) => (
                  <option key={scan.id} value={scan.id}>
                    {compactId(scan.id)} - {scan.status} - {scan.target_url ?? "target"}
                  </option>
                ))}
              </Select>
            </div>
            ...
            <Button disabled={!selectedProject || busy === "generate"} onClick={generateReport}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Reports are exported with hash metadata for audit traceability.</CardDescription>
        </CardHeader>
        <CardContent>
          {data.reports.length ? (
            <Table>
              ...
              <TableBody>
                {data.reports.map((report) => (
                  <TableRow key={report.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="font-mono text-xs text-muted-foreground">{compactId(report.id)}</p>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="font-mono text-xs">{compactId(report.report_hash)}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          disabled={busy === report.id || busy === `delete-${report.id}`}
                          onClick={() => void download(report.id, report.export_format)}
                        >
            ...
            <div className="rounded-lg border border-dashed p-10 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Generate a report after scan findings are available.</p>
            </div>
          )}
        </CardContent>
      </Card>
