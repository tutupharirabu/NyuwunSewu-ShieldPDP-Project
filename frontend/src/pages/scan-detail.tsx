...
function endpointDisplaySeverity(endpoint: EndpointInventory) {
  return (
    endpoint.highest_severity ?? endpoint.classifications[0]?.risk ?? "info"
  );
}
...
export function ScanDetailPage() {
    ...
    return (data?.endpoints ?? [])
      .filter((endpoint) => {
        ...
        const matchesSeverity =
          severity === "all" || displaySeverity === severity;
        const matchesSearch =
          !search ||
  ...
  if (error || !data) {
    return (
      <EmptyState
        ...
        description={error ?? "Unable to load scan inventory."}
      />
    );
  }

  const selected =
    data.endpoints.find((endpoint) => endpoint.id === selectedId) ??
    filteredEndpoints[0];
  const progress = Number(data.scan.stats.progress_percentage ?? 0);
  const criticalOrHigh = data.endpoints.filter((endpoint) =>
    ["critical", "high"].includes(
      endpointDisplaySeverity(endpoint).toLowerCase(),
    ),
  ).length;
  const diagnostics =
    typeof data.scan.stats.diagnostics === "object" &&
    data.scan.stats.diagnostics !== null
      ? (data.scan.stats.diagnostics as Record<string, unknown>)
      : {};
  const authentication =
    typeof diagnostics.authentication === "object" &&
    diagnostics.authentication !== null
  ...
  const contexts =
    typeof diagnostics.contexts === "object" && diagnostics.contexts !== null
      ...
      : {};
  const authenticationStatus = String(
    authentication.status ?? "not_configured",
  ).replace(/_/g, " ");
  const guestEndpoints = Number(contexts.guest?.endpoints_observed ?? 0);
  const authenticatedEndpoints = Number(
    contexts.authenticated?.endpoints_observed ?? 0,
  );
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-4">
              ...
              <div className="rounded-md border p-3">
                ...
                <p className="mt-1 text-2xl font-semibold">
                  {formatNumber(data.endpoints.length)}
                </p>
              </div>
              <div className="rounded-md border p-3">
                <p className="text-xs text-muted-foreground">Findings</p>
                <p className="mt-1 text-2xl font-semibold">
                  {formatNumber(data.findings.length)}
                </p>
              </div>
              <div className="rounded-md border p-3">
                <p className="text-xs text-muted-foreground">High/Critical</p>
                <p className="mt-1 text-2xl font-semibold">
                  {formatNumber(criticalOrHigh)}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 text-sm md:grid-cols-4">
              ...
              <div>
                ...
                {(guestEndpoints > 0 || authenticatedEndpoints > 0) && (
                  <p className="mt-1 text-xs text-muted-foreground">
                    {guestEndpoints} guest / {authenticatedEndpoints}{" "}
                    authenticated
                  </p>
                )}
              </div>
              <div>
                ...
                <p className="mt-1 font-medium">
                  {formatDate(data.scan.started_at)}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Finished</p>
                <p className="mt-1 font-medium">
                  {formatDate(data.scan.finished_at)}
                </p>
              </div>
            </div>
            {data.scan.error && (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Findings accepted for this scan after evidence checks.
            </CardDescription>
          </CardHeader>
            ...
            {data.findings.length ? (
              ...
              <div className="rounded-md border border-dashed p-6 text-center">
                ...
                <p className="mt-2 text-sm font-medium">
                  No accepted findings for this scan
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Review endpoint inventory and validation policy below.
                </p>
              </div>
            )}
          </CardContent>
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <CardDescription>
                Every crawled endpoint with classifications and finding linkage.
              </CardDescription>
            </div>
          ...
          {filteredEndpoints.length ? (
                    ...
                    >
                      <TableCell className="max-w-xl">
                        ...
                        <p className="mt-1 truncate text-xs text-muted-foreground">
                          {endpoint.title ?? endpoint.content_type ?? "-"}
                        </p>
                      </TableCell>
                      <TableCell>{endpoint.status_code ?? "-"}</TableCell>
                      <TableCell>
                        <Badge
                          variant={severityColor(displaySeverity) as never}
                        >
                          {displaySeverity}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          {endpoint.highest_confidence !== null && (
                            <span className="text-xs text-muted-foreground">
                              {Math.round(endpoint.highest_confidence)}%
                            </span>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="space-y-1">
                          <Badge variant="outline">
                            {primaryClass?.classification ?? "public"}
                          </Badge>
                          <p className="text-xs text-muted-foreground">
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {endpoint.query_parameters.length} params /{" "}
                        {endpoint.forms.length} forms
                      </TableCell>
                    </TableRow>
                  );
                })}
            ...
            <div className="rounded-lg border border-dashed p-10 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Clear the search or risk filter.
              </p>
            </div>
          )}
        </CardContent>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 lg:grid-cols-3">
              ...
              {selected.classifications.map((item, index) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <Badge
                      variant={
                        severityColor(String(item.risk ?? "info")) as never
                      }
                    >
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-md border p-3">
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {selected.query_parameters.length ? (
                    selected.query_parameters.map((item) => (
                ...
                <div className="mt-2 space-y-2">
                  {selected.forms.length ? (
                    selected.forms.map((form, index) => (
                      <div
                        key={`${selected.id}-form-${index}`}
                        className="rounded border p-2"
                      >
                        <p className="truncate text-xs font-medium">
                          {form.method ?? "GET"} {form.action ?? selected.url}
                        </p>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {(form.fields ?? [])
              ...
              <div className="rounded-md border p-3">
                <p className="text-xs text-muted-foreground">
                  Technologies and API profile
                </p>
                <div className="mt-2 flex flex-wrap gap-1">
                  {selected.tech_stack.length ? (
                    selected.tech_stack.map((item) => (
                      <Badge key={item} variant="outline">
                        {item}
                      </Badge>
                    ))
                    ...
                    <span className="text-xs text-muted-foreground">
                      No passive signature identified
                    </span>
                  )}
                </div>
              </div>
