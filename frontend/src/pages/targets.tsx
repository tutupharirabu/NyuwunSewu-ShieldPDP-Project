...
export function TargetsPage() {
  ...
  if (error || !data) return <EmptyState title="Targets unavailable" description={error ?? "Unable to load targets."} />;

  return (
    <Card>
      <CardHeader>
        <CardTitle>Targets</CardTitle>
        <CardDescription>Scoped target inventory and validation activity.</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="font-mono text-xs text-muted-foreground">{compactId(target.id)}</p>
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex flex-wrap gap-1">
                    {(target.allowed_domains.length ? target.allowed_domains : ["base host"]).slice(0, 3).map((domain) => (
